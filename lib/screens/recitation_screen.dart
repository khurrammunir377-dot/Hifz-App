...
class _RecitationScreenState extends State<RecitationScreen>
  ...
  AlignmentResult? _liveResult;
  String _teacherFeedback = 'Tap to check your recitation';
  ChunkStatus? _lastChunkStatus;
  String? _lastRecordingPath;
  ScoreReport? _finalScore;
  ...
  void initState() {
    ...
    _updatesSub = _checkService.updates.listen((update) {
      if (!mounted) return;
      setState(() {
        _liveResult = update.alignmentResult;
  ...
  Future<void> _startChecking() async {
    ...
    setState(() {
      ...
      _finalScore = null;
      _teacherFeedback = 'Listening...';
    });

    await _checkService.start(
      endpointUrl: _endpointUrl,
      expectedAyahs: [_currentAyah],
    );
  }

  Future<void> _stopChecking() async {
    final session = await _checkService.stop();
    setState(() {
      _isChecking = false;
      _liveResult = session.alignmentResult;
      _finalScore = session.score;
      _teacherFeedback = session.teacherFeedback;
      _lastRecordingPath = session.filePath;
      _showingFinalSummary = true;
    });

    // Keep the Settings stats screen (verse_progress table) in sync too.
    await DbHelper.instance.recordAttempt(
      surah: _currentAyah.surah,
      ayah: _currentAyah.ayah,
      accuracy: session.score.wordAccuracy,
    );
  }
  ...
  void _goToAyah(int delta) {
    ...
    setState(() {
      ...
      _lastRecordingPath = null;
      _showingFinalSummary = false;
      _teacherFeedback = 'Tap to check your recitation';
    });
    _saveSession();
  }
  ...
  Widget _buildAyahText(double fontSize) {
    ...
    if (_liveResult == null || displayWords.length != _liveResult!.wordStatus.length) {
      return Text(
        _currentAyah.text,
        textAlign: TextAlign.center,
    ...
    for (var i = 0; i < displayWords.length; i++) {
      final status = _liveResult!.wordStatus[i];
      Color color;
      switch (status) {
  ...
  Widget _buildMismatchDetails() {
    if (_liveResult == null || _liveResult!.errors.isEmpty) return const SizedBox.shrink();
    final relevant = _liveResult!.errors
        .where((e) => e.expectedWord != null && e.actualWord != null)
        .take(5)
        .toList();
      ...
      child: Column(
        children: relevant
            .map((e) => Padding(
                  ...
                  child: Text(
                    'Expected "${e.expectedWord}" — heard "${e.actualWord}"',
  ...
  Widget _buildCheckingControls() {
    String statusText = _teacherFeedback;
    if (_isChecking && _liveResult != null) {
      statusText = '${_liveResult!.correctCount}/${_liveResult!.totalWords} correct so far — $_teacherFeedback';
    } else if (_showingFinalSummary && _finalScore != null) {
      final pct = (_finalScore!.wordAccuracy * 100).toStringAsFixed(0);
      statusText = '$pct% word accuracy — $_teacherFeedback';
    }

    return Column(
      children: [
        Text(statusText, textAlign: TextAlign.center, style: const TextStyle(fontWeight: FontWeight.w600)),
        const SizedBox(height: 8),
        GestureDetector(
